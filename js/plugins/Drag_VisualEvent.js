...
/*:
 * @target MZ
 * @plugindesc (v0.1) An alternative node-based editor for common/map/troop events.
 * @author Drag
 *
 * @url
 *
 ...
 *
 * @param focusEditorOnOpen
 * @type boolean
 * @text Focus Editor On Open Play Test
 * @desc On: Focus will be on Event Editor when opening play test. Off: Focus will remain on game when opening play test.
 * @default false
 * @parent general
 *
 * @param backup
 * @text Back Up Options
 *
 * @param backupData
 * @type boolean
 * @text Back Up Data On Save
 * @desc On: Data backup will be created when saving in the visual event editor. Off: No backup created.
 * @default true
 * @parent backup
 *
 * @param backupDataSrc
 * @type string
 * @text Back Up Data Location
 * @desc Relative path where backup data should be stored (if enabled).
 * @default data/data_backup
 * @parent backup
 *
 * @param backupDataFormat
 * @type string
 * @text Back Up Data Format
 * @desc Backup file name template. $name (mandatory) => original file name. $year, $month, ect => backup creation date.
 * @default $name_$year-$month-$day-$hours-$minutes-$seconds
 * @parent backup
 *
 * @param autosave
 * @text Auto Save Options
 *
 * @param autoSaveEnabled
 * @type boolean
 * @text Auto Save Enabled
 * @desc On: Editor will automaticaly save at set time interval. Off: Editor will never automatically save.
 * @default true
 * @parent autosave
 *
 * @param autoSaveTimeInterval
 * @type number
 * @text Auto Save Time Interval
 * @desc The time interval in MINUTES to which the editor will repeatedly save, if enabled.
 * @default 5
 * @min 1
 * @parent autosave
 */
...
(function() {
	...
	//
	// IMPORTANT
	// SAVE AND RESTORE NODES POSITION
	// MAKE EDITOR SHORTCUTS (SAVE, UNDO, REDO)
	// IMPROVE PLUGIN COMMANDS PARAMETERS AND STRUCT INTERFACE
	
	// FIXES
	// FIX NESTED STRUCTURE PARMETERS MZ IN PLUGIN COMMANDS
	...
	// QOL
	// IMPROVE APPLY PLAYTEST FOR MAP EVENTS
	// QUICK ACCESS TO RECENTLY OPENED EVENTS
	
	// QOL ++
	// SWITCH PLUGINS PARAMETERS TO EDITOR CACHE ?
	// ADD INPUT CONTEXTMENU FOR RESTORE DEFAULT ? (MIGHT PREVENT COPY PASTING AND FIND ANOTHER WAY)
	// MAKE SOME OF THE BIG NODES BE SHRUNKABLE
	// MAKE GRAPH MOVE ON MOUSE POSITION WHEN ZOOMING 
	// BETTER RESPONSIVNESS OF EVERY WINDOWS
	// ADD A WAY TO LET USER ENABLE/DISABLE TEXTAREA AUTO RESIZE ?
	...
	// MISC
	// CLEAN CODE (never ending task)
	// IMPROVE GENERAL PERF & NODE CULLING
	
	// DONE 0.0.1
	// FIXES CONTEXTMENU SHOWING OUT OF SCREEN 
	// FIND BETTER WAY FOR USER TO SEE IF CHANGES ARENT SAVED/APPLIED YET
	// AUTO SAVE
	// SAVE ALL BUTTON
	// FIX PAGE HORIZONTAL SCROLL
	// ADD PAGE NAME INPUT
	// KEEP MAP CHANGES & DATA WHEN CHANGING MAP TO AVOID DATA LOSS
	// SAVE AND RESTORE LAST OPENED EVENT WHEN CLOSING/OPENING GRAPH
	// CLEAR GRAPH EDITOR WHEN LOADING OTHER MAP
	// SAVE AND RESTORE GRAPH POSITION WHEN CHANGING EVENTS
	// FIXE MOVE ROUTE IMAGE CHARACTER SUB SELECTION SIZE
	// FIXES SELECT COMBO ARROW KEYS LISTENERS (AGAIN)
	// MAKE TINT PICTURE/TINT SCREEN BUTTONS
	// SUPPORT FOR MV
	// FIX BATTLE BACKGROUND NOT SAVING SECOND BACKGROUND
	// IMPROVE SANITIZE INPUT NUMBER TO ALLOW FOR TEMPORARY NO-VALUE
	// FIX MAP EVENT SELECT (see set event location)
	// FIX MAP LOCATION PICKER (SEE COMMON EVENT 5)
	// FIX ADD NODE FROM LIST SPECIAL PARAMETERS (LIKE MOVE ROUTE)
	// FIX BIG CHARACTERS NOT APPEARING IN MAP LOCATION PREVIEW
	// FIX TILE EVENTS NOT APPEARING IN MAP LOCATION PREVIEW
	// FIXED PREVENT GRAPH ZOOM ON INPUT COMBO SELECT FOCUS
	// ADDED LOADING ROTATING SVG
	// FIX INPUT COMBO KEY LISTENERS
	// FIX DATABASE INPUT FIELD SET TO "UNDEFINED" OR "000:" INSTEAD OF NONE
	// FIX NODE CURVES ON ZOOMED IN/OUT
	// FIX NODE CULLING ON ZOOMED IN/OUT
	// FIX ADD NODE POSITION WITH RIGHT CLICK MENU WHEN ZOOMED IN/OUT
	// IMPLEMENT NODE CULLING
	// REMOVE LIGHT MODE SWITCH
	// ADD NODE CONTEXT MENU FOR UNDO/REDO
	// ADD NODE CONTEXTMENU KEY BINDS
	// ADD NODE CONTEXTMENU FOR COPY/PASTE/DELETE NODE, RESTORE ALL DEFAULT INPUTS, COPY/PASTE INPUT PARAMETERS
	// IMPROVE MOVE ROUTE EVENT SELECTION WITH CURRENTLY SELECTED MAP
	// ADD GRAB AND DRAG MERCHANDISE IN SHOP PROCESSING WINDOW
	// FIX COMMON EVENT 9 LOOP NOT CONNECTING TO FIRST SUBBRANCH
	// FIX ERROR COUNTER GOING UP EVEN THO THERE IS NO ERROR
	// CORRECT/BETTER PLACEMENT OF NODES WHEN OPENING EVENT
	// IMPROVE INTEGER/NUMBER INPUT FIELDS (INC/DEC ON MOUSE WHEEL ON FOCUS)
	// REMOVE HAVING TO PRESS CTRL WHEN MOVING SEVERAL NODES WITH MOUSE
	// REFRESH CURVES WHEN NODE IS RESIZED (by textarea or whatever)
	// FIX ADD NODE PARAMETERS WITH RIGHT CLICK MENU (show choices)
	// FIX IMAGE INPUT PREVIEW FOR TILESET & $files
	// SUPPORT FOR !$ IMAGES IN FILE EXPLORER
	// ADD TILESET SELECTION TO SINGLE FRAME CHARACTER (USED BY MAP EVENT)
	// MAKE ALL SHORTCUT WORKS OR REMOVE THEM
	// BACK UP DATA ON SAVE
	// CREATE EVENTS FROM LEFT PANEL LIST
	// FIX DATABASE FILTERING FUNCTION NOT WORKING CORRECTLY (SEE EVENT CONDITION)
	// ADD NONE (0) TO DATABASE INPUT FIELDS
	// ADD PAGE CONTEXT MENU (MOVE, COPY, PASTE, DELETE, CREATE)
	// FINISH SAVE/EXPORT
	// ADD CONDITIONS FOR TROOPS AND MAP EVENTS
	// ADD NOTETAG MANAGER FROM DEBUGGER
	// ADDED WARNING/ERROR ICON DISPLAY
	// ADD ALL DATAPOINTS FOR TROOPS AND MAP EVENTS
	// FIX BACKGROUND POSITION ON CHARACTERS/FACES/BATTLERS INPUTS
	// ADD LINKS TO ITCH AND PATREON
	// ADD BUTTON TO OPEN GAME CONSOLE & FOCUS PLAY TEST
	// ADD PARAMETER TO KEEP FOCUS ON PLAY TEST OR NOT 
	// ADD A WAY TO SHRINK/EXPAND MOVE ROUTE LIST SUMMARY
	// MAKE TOP PANEL FOR MAP EVENTS AND TROOP EVENTS
	// RESIZABLE LEFT PANEL
	// FIX COMMON EVENT 10 IF NODE WRONG VALUES 
	// FIX MISSING DIR ERROR IN FILE EXPLORER 
	// REMOVE INPUTS DRAGGABLE CURVES (not useful?) 
	// SELECTION BOX FOR NODES ON HOLD LEFT CLICK & MOVE 
	// CHANGE GRAPH EDITOR GRAB MOVE FROM LEFT TO RIGHT CLICK 
	// SELECT AND MOVE MULTIPLE NODE 
	// IMPROVE LEFT PANEL WITH EVENT TYPE SELECT AND A SEARCH INPUT FIELD 
	// SUPPORT FOR TROOPS EVENTS 
	// MAKE PAGES TABS FOR MAP EVENTS AND TROOP EVENTS
	// FINISH MOVE ROUTE PREVIEW 
	// MAKE SHOP PROCESSING INTERFACE 
	// IMPROVE DATABASE INPUT FIELDS 
	// FIX SHOW CHOICE SUBBRANCH CONNECTING TO WRONG OUTPUT (SEE COMMON EVENT 9)
	...
	Drag.VisualEvent.inputs = {
		...
		//conditions
		mapEventConditions: {type: "eventConditions", name: "Conditions", default: "", data: "data-eventType='Map Event'"},
		troopEventConditions: {type: "eventConditions", name: "Conditions", default: "", data: "data-eventType='Troop Event'"},
	...
	Drag.VisualEvent.getDefaultCommonEvent = function() {
		return Drag.VisualEvent.deepCopy(Drag.VisualEvent.defaultCommonEvent);
	};
	
	Drag.VisualEvent.getDefaultMapEvent = function() {
		return Drag.VisualEvent.deepCopy(Drag.VisualEvent.defaultMapEvent);
	};
	
	Drag.VisualEvent.getDefaultTroopEvent = function() {
		return Drag.VisualEvent.deepCopy(Drag.VisualEvent.defaultTroopEvent);
	};
	...
	Drag.VisualEvent.getCommandParameters = function(code) {
		...
		if (typeof code === "number")
			...
			console.log(code, Drag.VisualEvent.commandsParameters[code]);
		...
		for (const [i, match] of matches.entries()) {
			...
			if (tag === "command" || tag === "param" || isStruct) {
				...
				for (let j = i + 1; j < matches.length; j++) {
					...
					// console.log(`subTag:${subTag}, subVal:${subVal}`);
	...
	Drag.VisualEvent.reloadGame = function() {
		if (Drag.VisualEvent.graphWindowHandler)
			Drag.VisualEvent.graphWindowHandler.saveCache();
	...
	Drag.VisualEvent.mergeObjects = function(obj1, obj2, excludedKeys = []) {
		if (Array.isArray(obj1) && Array.isArray(obj2))
			...
			const merged = Drag.VisualEvent.deepCopy(obj1);
	...
	Drag.VisualEvent.mergeArrays = function(arr1, arr2, excludedKeys = []) {
		if (Array.isArray(arr1) && Array.isArray(arr2)) {
			const merged = Drag.VisualEvent.deepCopy(arr1);
	...
	Drag.VisualEvent.getDefaultItemPrice = function(itemType, itemId) {
		switch (itemType) {
			case 0:
				...
				break;
			case 1:
				return $dataWeapons[itemId].price;
				break;
			case 2:
				return $dataArmors[itemId].price;
				break;
		}
	};
		...
		try {
			if (!Drag.VisualEvent.graphWindowHandler) {
				if (fs.existsSync(Drag.VisualEvent.nwVisualEventWindowPath)) {
					...
					Drag.VisualEvent.graphWindowHandler.addEventListener("beforeunload", function(e) {
						console.log("Closing Visual Event Editor");
						Drag.VisualEvent.graphWindowHandler.saveCache();
						Drag.VisualEvent.graphWindowHandler = null;
					});
					
					// Drag.VisualEvent.graphWindowHandler.blur();
					// nw.Window.get().focus();
	...
	Drag.openWindow = function(path, commonDir, name, width, height, top, left, data = {}) {
		try {
			if (!Drag[`${name}WindowHandler`] || Drag[`${name}WindowHandler`].closed) {
				if (fs.existsSync(path)) {
					...
					// Drag[`${name}WindowHandler`].addEventListener("beforeunload", function(e) {
						// Drag[`${name}WindowHandler`] = null;
					// });
				} else { 
	...
	Drag.VisualEvent.openShopProcessingMenu = function(input) {
		...
		Drag.openWindow(
			...
			500, 900, rect.y + input.ownerDocument.defaultView.screenTop, rect.x + input.ownerDocument.defaultView.screenLeft, 
	...
	Drag.VisualEvent.openMoveRouteMenu = function(input) {
		...
		Drag.openWindow(
			...
			1100, 900, rect.y + input.ownerDocument.defaultView.screenTop, rect.x + input.ownerDocument.defaultView.screenLeft, 
	...
	Drag.VisualEvent.openFileExplorer = function(input) {
		...
		Drag.openWindow(
			...
			750, 900, rect.y + input.ownerDocument.defaultView.screenTop, rect.x + input.ownerDocument.defaultView.screenLeft, 
	...
	Drag.VisualEvent.openNotetagManager = function(input, eventData) {
		if (!eventData || !input)
			return;
		...
		Drag.openWindow(
			...
			500, 900, rect.y + input.ownerDocument.defaultView.screenTop, rect.x + input.ownerDocument.defaultView.screenLeft, 
			{input: input, eventData: eventData}
		);
	};
	...
	Drag.VisualEvent.openEventConditionsMenu = function(input) {
		...
		const rect = input.getBoundingClientRect();
		const eventId = Drag.VisualEvent.graphWindowHandler.data.targetId;
		const pageId = Drag.VisualEvent.graphWindowHandler.data.pageId || 0;
		const eventType = Drag.VisualEvent.graphWindowHandler.data.targetType;
		const eventData = eventType === "Map Event" ? Drag.VisualEvent.graphWindowHandler.data.loadedMap.events[eventId] : eventType === "Troop Event" ? Drag.VisualEvent.graphWindowHandler.data.$dataTroops[eventId] : null;
		Drag.openWindow(
			'html/Drag_DevTools_EventConditionsMenu.html', 'html/', 'Event Conditions', 
			720, 720, rect.y + input.ownerDocument.defaultView.screenTop, rect.x + input.ownerDocument.defaultView.screenLeft, 
			{input: input, eventType: eventType, eventData: eventData, pageId: pageId}
		);
	};
	...
	Drag.VisualEvent.onEditorCacheLoad = function(xhr) {
		if (xhr.status < 400) {
			...
			Drag.VisualEvent.graphWindowHandler.onCacheLoaded(data);
		} else
			this.onEditorCacheError();
	};
	
	Drag.VisualEvent.onEditorCacheError = function() {
		Drag.VisualEvent.graphWindowHandler.onCacheLoadError();
	};
		...
		fs.readFile(file, 'utf8', (err, data) => {
			if (err) {
				...
				if (Drag.VisualEvent.params.backupData)
					Drag.VisualEvent.createBackup(JSON.parse(data), filename);
	...
	Drag.VisualEvent.createBackup = function(data, name) {
		const date = new Date();
		...
		let userDefinedName = Drag.VisualEvent.params.backupDataFormat || "$name_$year-$month-$day-$hours-$minutes-$seconds";
		userDefinedName = 
		...
		let src = (Drag.VisualEvent.params.backupDataSrc || "data/data_backup/").trim();
		if (src[src.length - 1] !== "/")
			src = src + "/";
		
		const filename = `${src}${userDefinedName}.json`;
		Drag.VisualEvent.writeJSON(filename, data);
	};
	...
	Drag.VisualEvent.writeJSON = function(filename, obj, space = 4) {
		if (!obj || !filename)
			return;
	...
	Drag.VisualEvent.toggleCancelBranchChoice = function(input) {
		...
		if (input.value === "-2")
			Drag.VisualEvent.graphWindowHandler.showNodeOutput(node, 7);
		else
			Drag.VisualEvent.graphWindowHandler.hideNodeOutput(node, 7);
	};
	...
	Drag.VisualEvent.getDatabaseData = function(type) {
		const graphDatabaseData = Drag.VisualEvent.graphWindowHandler.data;
		if (type === "map_event")
			// return dataMap ? dataMap.events.filter(ev => ev) : [];
			return graphDatabaseData.loadedMap ? graphDatabaseData.loadedMap.events.filter(ev => ev) : [];
		else if (type === "switch" || type === "variable") {
			const dataType = Drag.VisualEvent.getDatabaseTypePlural(type).toLowerCase();
			return graphDatabaseData.$dataSystem[`${dataType}`] ? graphDatabaseData.$dataSystem[`${dataType}`].filter((data, index) => index > 0).map((data, index) => ({id: index + 1, name: data})) : [];
		} else if (type === "equipment_type")
			return graphDatabaseData.$dataSystem.equipTypes.slice(1).map((type, index) => ({name: type === "" ? `#${(index + 1).toString().padStart(2, "0")}` : type, id: index + 1}));
		else {
			const dataType = Drag.VisualEvent.capitalizeAll(Drag.VisualEvent.replaceAll(Drag.VisualEvent.getDatabaseTypePlural(type), "_", " "), ""); 
			return graphDatabaseData[`$data${dataType}`] ? graphDatabaseData[`$data${dataType}`].filter(data => data) : [];
		}
	};
	...
	Drag.VisualEvent.escapeQuotes = function(string = "") {
		if (typeof string === "string")
			string = string.replace(/"/g, '&quot;').replace(/'/g, "&apos;")
		return string;
	};
	
	Drag.VisualEvent.unescapeQuotes = function(string = "") {
		if (typeof string === "string")
			string = string.replace(/&quot;/g, '"').replace(/&apos;/g, "'");
		return string;
	};
	...
	Drag.VisualEvent.getListInputButtons = function() {
		return `
			<span style="cursor: pointer; height: 24px;" onclick="this.parentElement.after(this.parentElement.cloneNode(true));">
			...
			<span style="cursor: pointer; height: 24px;" onclick="if (this.parentElement.parentElement.childElementCount > 1) this.parentElement.remove();">
	...
	Drag.VisualEvent.getInputValue = function(input) {
		...
		for (const [i, value] of values.entries()) {
			if (input.type === "number" || input.getAttribute('data-inputType') === "range" || input.getAttribute('data-inputType') === "integer" || input.getAttribute('data-inputType') === "number" || input.getAttribute('data-dataType') === "number")
				...
				values[i] = input.checked ? value : null;
	...
	Drag.VisualEvent.getPluginCommandInputValue = function(input) {
		...
		if (inputType === "location") {
			...
			return JSON.parse(input.getAttribute('data-structValue'));
		else
	...
	Drag.VisualEvent.getInputField = function(params, index = null, controller = null) {
		...
		if (params.type === "struct")
			params.value = Drag.VisualEvent.escapeQuotes(JSON.stringify(params.value));
		...
		if ((params.value === "" || params.value === undefined) && params.default)
			if (params.isList)
				params.value = JSON.parse(params.default);
			else
	...
	Drag.VisualEvent.getDefaultInputField = function(params, index, controller = null) {
		return `<input type="text" class="${params.class || ''}" value="${params.value || params.default || ''}" ${params.data || ''} ${!params.notParam ? 'data-isCommandParameter="true"' : ''} ${params.disabled ? 'disabled' : ''} />`;
	};
	...
	Drag.VisualEvent.getStructInputField = function(params, index, controller = null) {
		return `<input 
			...
			${params.data || ''} ${!params.notParam ? 'data-isCommandParameter="true"' : ''} data-structValue="${params.value || params.default || '{}'}" data-pluginName="${params.pluginName}" data-structName="${params.structName}" data-noReset="true"
			onclick="this.blur(); $.Drag.VisualEvent.openStructureManager(this);"
		/>`;
	};
	
	Drag.VisualEvent.getTextInputField = function(params, index, controller = null) {
		return `
			<textarea onchange="${params.onchange || ''} $.Drag.VisualEvent.autoFitTextArea(this);" onkeyup="this.onchange();" onpaste="this.onchange();" oninput="this.onchange();" 
	...
	Drag.VisualEvent.getStringInputField = function(params, index, controller = null) {
		return `
			<input onchange="${params.onchange || ''}" onkeyup="this.onchange();" onpaste="this.onchange();" oninput="this.onchange();" 
		...
		return `
			<div class="relative">
				<input type="number" id="${params.id ? params.id : ''}" class="${params.class || ''}" 
					value="${params.value || params.default || 0}" ${params.max !== undefined ? `max="${params.max}"` : ''} ${params.min !== undefined ? `min="${params.min}"` : ''} placeholder="${params.placeholder || ''}" 
	...
	Drag.VisualEvent.getDatabaseInputField = function(params, index, controller = null) {
		...
		// const id = Math.max(Math.min(params.value !== undefined ? params.value : defaultId, databaseOptions.length), -params.addOptions.length + 1);
		...
		return `
			<div class="relative">
				...
				<select 
					...
					onchange="$.Drag.VisualEvent.onSelectComboChange(this);" onclick="this.onchange();"
	...
	Drag.VisualEvent.handleComboSelectClickOutside = function(e) {
		...
		if (e.target !== input && e.target !== this && !e.path.includes(input) && !e.path.includes(this)) {
			...
			const option = Array.from(this.options).find(option => option.value === value); //this.options[parseInt(input.getAttribute('data-value'))];
			if (option) {
				input.value = option.text;
				input.setAttribute('data-value', option.value);
			}
			
			Drag.VisualEvent.hideSelect(this);
	...
	Drag.VisualEvent.onSelectComboChange = function(select) {
		...
		input.value = select.options[select.options.selectedIndex].text; 
	...
	Drag.VisualEvent.getComboInputField = function(params, index, controller = null) {
		return `
			<div class="relative">
				<input
					type="text" class="" value="${params.value || params.default || ''}"  placeholder="${params.placeholder || ''}"
				...
				>
				<select 
					onchange="this.previousElementSibling.value = this.options[this.options.selectedIndex].text; this.style.display = 'none';"
	...
	Drag.VisualEvent.getRangeInputField = function(params, index, controller = null) {
		return `
			<div class="relative columnGap05em flex" style="flex-direction: row; align-items: center;">
				<input type="range" min="${params.min}" max="${params.max}" step="${params.step}" class="${params.class || ''}" value="${params.value || params.default || 0}"
					onchange="this.nextElementSibling.firstElementChild.value = this.value; ${params.onchange || ''}" oninput="this.onchange();"  ${params.data || ''} ${params.disabled ? 'disabled' : ''}/>
				<div class="relative">
					<input type="number" min="${params.min}" max="${params.max}" step="${params.step}" class="onReadyOnChange ${params.class || ''}" style="max-width: 75px;" value="${params.value || 0}" placeholder="${params.placeholder || ''}" 
	...
	Drag.VisualEvent.getColorInputField = function(params, index, controller = null) {
		return `
			<div class="relative columnGap05em flex">
				<input type="color" class="${params.class || ''}" style="width: 75px; height: 75px; cursor: pointer;"
				...
				/>
				<div style="position: absolute; width: 81px; height: 77px; pointer-events: none; border: white 5px solid; box-sizing: border-box; top: 1px;"></div>
				${params.alphaAsGrey ? `
				<div class="relative" style="display: grid; grid-template-columns: 1fr 0.5fr; grid-template-rows: 0.5fr 0.5fr; align-items: center;">
					<span style="margin-left: 10px; margin-bottom: -20px;">Gray :</span>
					<br>
					...
					<div class="relative">
						<input type="number" min="0" max="255" step="5" class=" ${params.class || ''}" style="max-width: 75px;" value="${params.value[3] || 0}" data-inputType="integer" data-defaultValue="255"
	...
	Drag.VisualEvent.getFileInputField = function(params, index, controller = null) {
		...
		return `
			<input type="text" class=""
				style="cursor: pointer;" 
				onclick="$.Drag.VisualEvent.openFileExplorer(this);" onchange="" onfocus="this.blur()" onload=""
	...
	Drag.VisualEvent.getAudioInputField = function(params, index, controller = null) {
		return `
			<input type="text" class=""
				style="cursor: pointer;" 
				onclick="$.Drag.VisualEvent.openFileExplorer(this); ${params.onclick || ''}" onchange="" onfocus="this.blur()" onload=""
				${params.data || ""} ${!params.notParam ? 'data-isCommandParameter="true"' : ''} value="${params.value || ''}"
			/>`;
	};
	...
	Drag.VisualEvent.getImageInputField = function(params, index, controller = null) {
		...
		return `
			<input type="text" class="onReadyOnChange"
				style="font-size: 0; cursor: pointer; padding: 0; position: relative; overflow: hidden; width: ${width}px; height: ${height}px; background-size: cover;" 
				onclick="$.Drag.VisualEvent.openFileExplorer(this); ${params.onclick || ''}" onchange="$.Drag.VisualEvent.updatePicture(this)" onfocus="this.blur()" onload="$.Drag.VisualEvent.updatePicture(this)"
				${params.data || ""} ${!params.notParam ? 'data-isCommandParameter="true"' : ''} value="${params.value}"
			/>`;
	};
	...
	Drag.VisualEvent.getShopProcessingInputField = function(params, index, controller = null) {
		return `
			<button class="${params.class || ''}" style="cursor: pointer;" 
				onclick="${params.onclick || ''} $.Drag.VisualEvent.openShopProcessingMenu(this);" onchange=""
				${params.data || ""} ${!params.notParam ? 'data-isCommandParameter="true"' : ''} data-value="${params.value || params.default}"
	...
	Drag.VisualEvent.getLocationInputField = function(params, index, controller = null) {
		...
		// const mapId = params.value.length > 2 ? params.value[0] : params.mapId ? params.mapId : 0;
		...
		return `
			<input type="text" class="" style="cursor: pointer;" 
	...
	Drag.VisualEvent.getEventConditionsInputField = function(params, index, controller = null) {
		...
		return `
			<button class="${params.class || ''}" style="cursor: pointer;" 
				onclick="$.Drag.VisualEvent.openEventConditionsMenu(this); ${params.onclick || ''}" onchange="$.Drag.VisualEvent.updateEventConditionsList(this);" ${params.data || ""} ${!params.notParam ? 'data-isCommandParameter="true"' : ''} 
			>Set Conditions...</button>
			${params.showSummary !== false ? Drag.VisualEvent.getEventConditionsInputList(conditions) : ''}`;
	};
	
	Drag.VisualEvent.getEventConditionsInputList = function(conditions) {
		return `
			<div id="event-conditions-list"> 
				<ul class="shrinkable">
					${conditions.map(condition => `<li>${Drag.VisualEvent.translateConditions(condition)}</li>`).join('')}
				</ul>
				<div class="shrink-symbol" onclick="this.previousElementSibling.classList.toggle('shrink');">
					<span>&#x2771;</span>
				</div>
	...
	Drag.VisualEvent.updateEventConditionsList = function(button) {
		const eventId = Drag.VisualEvent.graphWindowHandler.data.targetId;
		const pageId = Drag.VisualEvent.graphWindowHandler.data.pageId || 0;
		const eventType = Drag.VisualEvent.graphWindowHandler.data.targetType;
		const conditions = eventType === "Map Event" ? Drag.VisualEvent.graphWindowHandler.data.loadedMap.events[eventId].pages[pageId].conditions : eventType === "Troop Event" ? Drag.VisualEvent.graphWindowHandler.data.$dataTroops[eventId].pages[pageId].conditions : null;
		
		const listElement = button.nextElementSibling;
		if (listElement)
			listElement.outerHTML = Drag.VisualEvent.getEventConditionsInputList(Drag.VisualEvent.formatConditions(conditions));
	};
